...
use app\commodity\model\CommodityList;
use app\commodity\model\CommodityModelField;
use app\commodity\model\CommodityType;
use app\common\controller\Base;
use app\commodity\model\CommodityModelList;
use app\complain\model\ComplainCommodity;
...
class Commodity extends Base
{
    ...
    public function modelAdd(){
        if ($this->request->isAjax()){
            ...
            //验证
            $validate = new \app\commodity\validate\CommodityModelList();
            if (!$validate->check($data)){
                return ajaxReturnError($validate->getError());
            }
            //创建附加数据表
            $db_name = Config::get('database.prefix')."commodity_with_".$data['db_name'];
            $info = "(uid int(11) NOT NULL,PRIMARY KEY (uid)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='{$data['name']}商品模型附加表'";
            ...
            //写入数据
            $r = CommodityModelList::create($data);
            return ajaxReturn($r);
        }
        return $this->fetch();
    }
    ...
    public function modelEdit(){
        if ($this->request->isAjax()){
            ...
            $r = CommodityModelList::update($data);
            return ajaxReturn($r);
        }

        $id = $this->request->param('cid');
        $r = CommodityModelList::get($id);
        $this->assign('data',$r);
        return $this->fetch();
    }
    ...
    public function modelDelete(){
        if ($this->request->isAjax()){
            ...
            //判断是否是系统模型
            $data = CommodityModelList::get($par);
            if ($data['is_system']){
                return ajaxReturnError('系统模型,无法删除');
            }
            //判断是否有栏目使用此模型
            $is_child = CommodityType::where('model_id',$par)->find();
            if ($is_child){
    ...
    public function fieldList(){
        ...
        //模型信息
        $model_info = CommodityModelList::get($mid);
        //模型下的字段信息
        $ret = CommodityModelField::where('model_id',$mid)
            ->with('withForm')
            ->order('order')
            ->all();
    ...
    public function fieldAdd(){
        ...
        if ($this->request->isAjax()){
            $data = $this->request->param();
            //验证
            $validate = new \app\commodity\validate\CommodityModelField();
            if (!$validate->check($data)){
                return ajaxReturnError($validate->getError());
            }
            ...
            $r = CommodityModelField::create($data);
            return ajaxReturn($r);
        }
    ...
    public function fieldEdit(){
        if ($this->request->isAjax()){
            ...
            $r = CommodityModelField::update($data);
            return ajaxReturn($r);
        }

        $id = $this->request->param('cid');
        $r = CommodityModelField::with('withModel')->get($id);
        $this->assign('data',$r);
        return $this->fetch();
    }
    ...
    public function fieldDelete(){
        if ($this->request->isAjax()){
            $par = $this->request->param('id');
            $data= CommodityModelField::with('withModel')->get($par);
            //删除附加表中的字段
            $sql_mode = new \vancens\mysql\MysqlManage();
    ...
    public function typeEdit(){
        if ($this->request->isAjax()){
            $data = $this->request->param();
            $r = CommodityType::update($data);
            return ajaxReturn($r);
        }

        $id = $this->request->param('cid');
        $r = CommodityType::get($id);
        $this->assign('data',$r);
        return $this->fetch();
    }
    ...
    public function typeDelete(){
        if ($this->request->isAjax()){
            $par = $this->request->param('id');
            //判断是否有子栏目
            $is_child = CommodityType::where('pid',$par)->find();
            if ($is_child){
                return ajaxReturnError('当前分类下存在子分类,无法删除');
            }
            $ret = CommodityType::destroy($par);
            return ajaxReturn($ret);
        }
    }
    ...
    public function listsByType(){
        ...
        $mode = new CommodityList();
        $data = $mode
            ->where('type_id',$tid)
            ->with(['withType','withStore','withStatus'])
            ->select();
        $this->assign('data',$data);
        //分组信息
        $typeInfo = CommodityType::get($tid);
    ...
    public function add(){
        if ($this->request->isAjax()){
            ...
            try {
                //写入主表数据
                $content_save = Db::name('commodity_list')->strict(false)->insertGetId($par);
                //获取附加表名称
                $db_name = Db::name('commodity_model_list')->where('id',$par['type_model_id'])->field('db_name')->find();
                $db_name_full = Config::get('database.prefix').'commodity_with_'.$db_name['db_name'];
    ...
    public function edit(){
        if ($this->request->isAjax()){
            ...
            $par['update_time'] = time();
            $par['audit_time'] = time();
            $par['audit_adminer'] = $this->_GLOBAL_ADMIN['id'];
            //多选按钮值转为字符串存储
            foreach ($par as $key=>$value){
                if (is_array($value)){
                    if (count($value) > 1){
            ...
            Db::startTrans();
            try {
                Db::name('commodity_list')
                    ->where('id',$par['id'])
                    ->strict(false)
                    ->update($par);
                Db::table($par['db_name'])
                    ->strict(false)
                    ->where('uid',$par['id'])
                    ->update($par);
                Db::commit();
        ...
        //读取栏目数据
        $typeInfo = CommodityType::with('withModel')->get($data['type_id']);
        $this->assign('type',$typeInfo);
    ...
    public function del(){
        if ($this->request->isAjax()){
            ...
            try {
                //栏目ID
                $type_id = Db::name('commodity_list')->where('id',$id)->value('type_id');
                //模型ID
                $model_id  = Db::name('commodity_type')->where('id',$type_id)->value('model_id');
                if ($model_id){
                    //附加表名称
                    $db_name   = Db::name('commodity_model_list')->where('id',$model_id)->value('db_name');
                    $db_name   = Config::get('database.prefix').'commodity_with_'.$db_name;
                    Db::table($db_name)->where('uid',$id)->delete();
                }

                Db::name('commodity_list')->delete($id);
                Db::commit();
